...
public class ElementsKraftSingles implements IFuelHandler, IWorldGenerator {
	...
	public ElementsKraftSingles() {
		...
		sounds.put(new ResourceLocation("kraftsingles", "cursedsounds"),
				new net.minecraft.util.SoundEvent(new ResourceLocation("kraftsingles", "cursedsounds")));
		...
		sounds.put(new ResourceLocation("kraftsingles", "push-button-sound"),
				new net.minecraft.util.SoundEvent(new ResourceLocation("kraftsingles", "push-button-sound")));
		sounds.put(new ResourceLocation("kraftsingles", "out-of-touch"),
				new net.minecraft.util.SoundEvent(new ResourceLocation("kraftsingles", "out-of-touch")));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		try {
			for (ASMDataTable.ASMData asmData : event.getAsmData().getAll(ModElement.Tag.class.getName())) {
				...
				if (clazz.getSuperclass() == ElementsKraftSingles.ModElement.class)
					elements.add((ElementsKraftSingles.ModElement) clazz.getConstructor(this.getClass()).newInstance(this));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		Collections.sort(elements);
		elements.forEach(ElementsKraftSingles.ModElement::initElements);
		this.addNetworkMessage(KraftSinglesVariables.WorldSavedDataSyncMessageHandler.class, KraftSinglesVariables.WorldSavedDataSyncMessage.class,
	...
	public void onPlayerLoggedIn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.player.world.isRemote) {
			WorldSavedData mapdata = KraftSinglesVariables.MapVariables.get(event.player.world);
			WorldSavedData worlddata = KraftSinglesVariables.WorldVariables.get(event.player.world);
			if (mapdata != null)
				KraftSingles.PACKET_HANDLER.sendTo(new KraftSinglesVariables.WorldSavedDataSyncMessage(0, mapdata), (EntityPlayerMP) event.player);
			if (worlddata != null)
				KraftSingles.PACKET_HANDLER.sendTo(new KraftSinglesVariables.WorldSavedDataSyncMessage(1, worlddata), (EntityPlayerMP) event.player);
		}
	}
	...
	public <T extends IMessage, V extends IMessage> void addNetworkMessage(Class<? extends IMessageHandler<T, V>> handler, Class<T> messageClass,
			Side... sides) {
		for (Side side : sides)
			KraftSingles.PACKET_HANDLER.registerMessage(handler, messageClass, messageID, side);
		messageID++;
	}
	...
	public static class ModElement implements Comparable<ModElement> {
		...
		protected final ElementsKraftSingles elements;
		protected final int sortid;
		public ModElement(ElementsKraftSingles elements, int sortid) {
